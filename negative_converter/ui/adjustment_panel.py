...
class AdjustmentPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        icon = QIcon.fromTheme("color-picker", QIcon(":/icons/color-picker.svg")) # Example path
        if icon.isNull():
             self.wb_picker_button.setText("WB Pick")
             print("Warning: Could not find color-picker icon for WB button.")
        else:
             self.wb_picker_button.setIcon(icon)
             self.wb_picker_button.setIconSize(QSize(16, 16)) # Adjust size as needed
