...
class AdjustmentPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        # --- Add Reset/Undo/Redo Buttons at the top (outside scroll area) ---
        top_button_layout = QHBoxLayout()
        top_button_layout.setContentsMargins(5, 5, 5, 5)
        self.undo_button = QPushButton("Undo")
        self.undo_button.setToolTip("Undo last adjustment change (Ctrl+Z)")
        self.undo_button.clicked.connect(self.undo_adjustment)
        self.undo_button.setEnabled(False)
        self.undo_button.setMaximumWidth(60)
        top_button_layout.addWidget(self.undo_button)

        self.redo_button = QPushButton("Redo")
        self.redo_button.setToolTip("Redo last undone adjustment change (Ctrl+Y)")
        self.redo_button.clicked.connect(self.redo_adjustment)
        self.redo_button.setEnabled(False)
        self.redo_button.setMaximumWidth(60)
        top_button_layout.addWidget(self.redo_button)

        top_button_layout.addStretch(1)

        self.reset_all_button = QPushButton("Reset All")
        self.reset_all_button.setToolTip("Reset all adjustments to default values")
        self.reset_all_button.clicked.connect(self.reset_adjustments)
        self.reset_all_button.setMaximumWidth(80)
        top_button_layout.addWidget(self.reset_all_button)

        outer_layout.addLayout(top_button_layout)
        
        # --- Create scroll area for all adjustment groups ---
