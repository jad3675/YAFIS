...
class FilmPresetPanel(QWidget): # Renamed back
    ...
    def trigger_preview(self):
        ...
        if hasattr(main_window, 'get_current_image_for_processing'):
            current_image = main_window.get_current_image_for_processing()
        else:
            print("Preview trigger: Main window missing 'get_current_image_for_processing' method.")
            return # Cannot proceed without image source

        if current_image is None:
            print("Preview trigger: No current image available in main window.")
        ...
        if self.current_preset_id:
            ...
            self.preview_requested.emit(current_image, 'film', self.current_preset_id, self.intensity, self.grain_scale)
        else:
            ...
            self.preview_requested.emit(current_image, None, None, 0.0, 0.0)
    ...
    def trigger_apply(self):
        ...
        if self.current_preset_id:
            ...
            # Get current image from main window using stored reference
            main_window = self.main_window
            if hasattr(main_window, 'get_current_image_for_processing'): # Check if method exists
                 current_image = main_window.get_current_image_for_processing()
                 if current_image is not None:
                    # Emit signal with type 'film'
                    self.apply_requested.emit(current_image, 'film', self.current_preset_id, self.intensity, self.grain_scale)
                 else:
                     print("Apply trigger: No current image available in main window.")
            else:
                 print("Apply trigger: Main window does not have 'get_current_image_for_processing' method.")
        else:
